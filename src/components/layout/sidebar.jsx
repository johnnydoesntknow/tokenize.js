...
const Sidebar = ({ isCollapsed, toggleSidebar, activeView, setActiveView, isMobileMenuOpen, setIsMobileMenuOpen }) => {
  const { address, isConnected, signer, isAdminUser } = useWeb3(); // Add isAdminUser
  ...
  return (
    <>
      ...
      `}>
          ...
          <div className={`flex items-center ${isCollapsed && !isMobileMenuOpen ? 'lg:justify-center' : 'gap-3'}`}>
            ...
            {(!isCollapsed || isMobileMenuOpen) && (
              <div>
                ...
                <p className="text-neutral-500 text-xs font-light">Tokenization</p>
              </div>
            )}
          </div>
        </div>
        ...
        {isAdminUser && (
          <div className={`mx-3 mt-4 px-3 py-2 bg-yellow-900/20 border border-yellow-600/20 rounded ${isCollapsed && !isMobileMenuOpen ? 'lg:hidden' : ''}`}>
            <div className="flex items-center gap-2">
              <Shield className="w-4 h-4 text-yellow-400" />
              {(!isCollapsed || isMobileMenuOpen) && (
                <span className="text-xs text-yellow-400">Admin Access</span>
              )}
            </div>
          </div>
        )}
