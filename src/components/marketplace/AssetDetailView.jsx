  ...
  useEffect(() => {
    const fetchTransactions = async () => {
      ...
      try {
        ...
        const investors = await Tokenization.getAssetInvestors(asset.assetId);
        ...
        if (investors && investors.length > 0) {
          ...
          for (const investor of investors) {
            try {
              const shares = await Tokenization.getUserShares(investor, asset.assetId);
              const ownershipData = await Tokenization.getUserOwnershipPercentage(investor, asset.assetId);
