// test/OPNTokenization.test.js
const { expect } = require("chai");
const { ethers } = require("hardhat");

describe("OPN Tokenization Platform", function () {
  let Tokenization;
  let kycRegistry;
  let owner;
  ...
  beforeEach(async function () {
    ...
    // Deploy OPNTokenization
    const OPNTokenization = await ethers.getContractFactory("OPNTokenization");
    Tokenization = await OPNTokenization.deploy(
      BASE_URI,
    ...
    // Setup roles
    const COMPLIANCE_ROLE = await Tokenization.COMPLIANCE_ROLE();
    const KYC_VERIFIER_ROLE = await kycRegistry.KYC_VERIFIER_ROLE();

    await Tokenization.grantRole(COMPLIANCE_ROLE, complianceOfficer.address);
  ...
  describe("Tokenization Requests", function () {
    beforeEach(async function () {
      await kycRegistry.connect(kycVerifier).verifyKYC(user1.address, "QmTestHash123", 0);
    });

    it("Should create a Tokenization request", async function () {
      await expect(
        Tokenization.connect(user1).createTokenizationRequest(
          "Luxury Watch",
      ...
      ).to.emit(Tokenization, "RequestCreated");

      const requests = await Tokenization.getUserRequests(user1.address);
      expect(requests.length).to.equal(1);
    });

    it("Should not allow non-KYC verified user to create request", async function () {
      await expect(
        Tokenization.connect(user2).createTokenizationRequest(
  ...
  describe("Request Approval/Rejection", function () {
    ...
    beforeEach(async function () {
      ...
      const tx = await Tokenization.connect(user1).createTokenizationRequest(
        "Luxury Watch",
        "Patek Philippe Nautilus 5711",
        "Iconic luxury sports watch",
        "https://example.com/watch.jpg",
    ...
    it("Should approve request and mint tokens", async function () {
      await expect(
        Tokenization.connect(complianceOfficer).approveRequest(requestId)
      ).to.emit(Tokenization, "RequestApproved");

      const request = await Tokenization.requests(requestId);
      expect(request.status).to.equal(1); // Approved

      const assetDetails = await Tokenization.assetDetails(0);
    ...
    it("Should reject request", async function () {
      await expect(
        Tokenization.connect(complianceOfficer).rejectRequest(requestId, "Insufficient documentation")
      ).to.emit(Tokenization, "RequestRejected");
    ...
    it("Should not allow non-compliance officer to approve", async function () {
      await expect(
        Tokenization.connect(user1).approveRequest(requestId)
      ).to.be.reverted;
    });
  });
  ...
  describe("Marketplace - Purchase Fractions", function () {
    ...
    beforeEach(async function () {
      // Setup: Create and approve a Tokenization request
      await kycRegistry.connect(kycVerifier).verifyKYC(user1.address, "QmTestHash123", 0);
      await kycRegistry.connect(kycVerifier).verifyKYC(user2.address, "QmTestHash123", 0);
      
      await Tokenization.connect(user1).createTokenizationRequest(
        "Luxury Watch",
        "Patek Philippe Nautilus 5711",
        "Iconic luxury sports watch",
        "https://example.com/watch.jpg",
      ...
      await Tokenization.connect(complianceOfficer).approveRequest(0);
      tokenId = 0;
    });
    ...
    it("Should allow KYC verified user to purchase fractions", async function () {
      ...
      await expect(
        Tokenization.connect(user2).purchaseFractions(tokenId, purchaseAmount, { value: totalCost })
      ).to.emit(Tokenization, "FractionsPurchased");
      
      // Check buyer received fractions
      expect(await Tokenization.balanceOf(user2.address, tokenId)).to.equal(purchaseAmount);
      
      // Check asset details updated
      const assetDetails = await Tokenization.assetDetails(tokenId);
    ...
    it("Should not allow non-KYC user to purchase when KYC required", async function () {
      ...
      await expect(
        Tokenization.connect(nonKycUser).purchaseFractions(tokenId, 10, { value: fractionPrice * BigInt(10) })
    ...
    it("Should refund excess payment", async function () {
      ...
      const tx = await Tokenization.connect(user2).purchaseFractions(
        tokenId, 
        purchaseAmount, 
  ...
  describe("Admin Functions", function () {
    it("Should update platform fee", async function () {
      const newFee = 500; // 5%
      await Tokenization.updatePlatformFee(newFee);
      expect(await Tokenization.platformFee()).to.equal(newFee);
    });

    it("Should not allow fee above 10%", async function () {
      await expect(
        Tokenization.updatePlatformFee(1001)
      ).to.be.revertedWith("Fee too high");
    });

    it("Should pause and unpause contract", async function () {
      await Tokenization.pause();
      ...
      ).to.be.revertedWithCustomError(Tokenization, "EnforcedPause");

      await Tokenization.unpause();
