...
// COMPLETE FILE - With IPFS folder image fetching integrated
...
import { fetchImagesFromIPFSFolder } from '../../utils/ipfsUpload';
...
const AssetDetailView = ({ asset, onBack }) => {
  ...
  // ============================================================================
  // NEW: Fetch images from IPFS folder
  // ============================================================================
  useEffect(() => {
    const fetchAssetImages = async () => {
      ...
      try {
        ...
        console.log('🖼️ Fetching images for asset:', asset.assetName);
        
        let imageURLs = [asset.assetImageUrl]; // Start with main image
        
        // Try to get metadata first
        if (asset.metadataUrl) {
          try {
            const gatewayURL = import.meta.env.VITE_IPFS_GATEWAY || 'https://ipfs.io/ipfs/';
            const metadataResponse = await fetch(`${gatewayURL}${asset.metadataUrl}`);
            const metadata = await metadataResponse.json();
            
            console.log('📄 Asset metadata:', metadata);
            
            // If metadata has folder CID, fetch all images from folder
            if (metadata.ipfsFolder) {
              console.log('📁 Found IPFS folder:', metadata.ipfsFolder);
              const folderImages = await fetchImagesFromIPFSFolder(metadata.ipfsFolder);
              
              if (folderImages && folderImages.length > 0) {
                imageURLs = folderImages.map(img => img.url);
                console.log('✅ Loaded images from IPFS folder:', imageURLs);
              }
            }
            // If metadata has images array (old format), use those
            else if (metadata.images && metadata.images.length > 0) {
              imageURLs = [metadata.mainImage, ...metadata.images].filter(Boolean);
              console.log('📋 Found images in metadata array:', imageURLs);
            }
          } catch (metadataError) {
            console.warn('⚠️ Could not fetch metadata, falling back to description parsing:', metadataError);
            
            // Fallback: try parsing from description (old format)
            const parsedImages = parseAdditionalImages(asset.assetDescription);
            if (parsedImages.length > 0) {
              imageURLs = [asset.assetImageUrl, ...parsedImages].filter(Boolean);
              console.log('📋 Found images in description:', imageURLs);
            }
          }
        } else {
          // No metadata URL, try parsing from description
          const parsedImages = parseAdditionalImages(asset.assetDescription);
          if (parsedImages.length > 0) {
            imageURLs = [asset.assetImageUrl, ...parsedImages].filter(Boolean);
            console.log('📋 Found images in description:', imageURLs);
        ...
        setAllImages([asset.assetImageUrl]);
      } finally {
          ...
          <>
            ...
            {allImages.length > 1 && (
              <>
                ...
                <button
                  onClick={prevImage}
                  className="absolute left-4 top-1/2 -translate-y-1/2 p-3 bg-black/50 hover:bg-black/70 backdrop-blur-sm rounded-lg transition-all opacity-0 group-hover:opacity-100"
                >
                ...
                <button
                  onClick={nextImage}
                  className="absolute right-4 top-1/2 -translate-y-1/2 p-3 bg-black/50 hover:bg-black/70 backdrop-blur-sm rounded-lg transition-all opacity-0 group-hover:opacity-100"
                >
                  <ChevronRight className="w-6 h-6 text-white" />
                </button>
                
                {/* Image Counter */}
                <div className="absolute top-4 right-4 px-3 py-1 bg-black/70 backdrop-blur-sm rounded-lg text-white text-sm">
                ...
                <div className="absolute bottom-20 left-1/2 -translate-x-1/2 flex gap-2">
                  ...
                  {allImages.length > 10 && (
                    <span className="text-white/50 text-xs">+{allImages.length - 10}</span>
                  )}
                </div>
              </>
            )}
            ...
            <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
              <div>
                <div className="flex items-center gap-3 mb-3 flex-wrap">
                  <span className="px-3 py-1 bg-white/10 backdrop-blur-sm text-xs font-normal text-white/80 rounded-lg">
                    {asset.assetType}
                  </span>
              ...
              {allImages.length > 1 && (
                <button
                  ...
                  className="px-4 py-2 bg-white/10 hover:bg-white/20 backdrop-blur-sm text-white text-sm font-light rounded-lg transition-colors flex items-center gap-2 pointer-events-auto"
                >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            {activeTab === 'documents' && (
              <div className="space-y-4">
                <div className="bg-neutral-950 border border-neutral-900 p-8 rounded-lg text-center">
                ...
                {loadingTransactions ? (
                  ...
                  <div className="space-y-2">
                    {transactions.map((tx, idx) => (
                      <div key={idx} className="bg-neutral-950 border border-neutral-900 p-4 rounded-lg">
          ...
          <div className="lg:col-span-1">
            <div className="sticky top-6 bg-neutral-950 border border-neutral-900 p-6 rounded-lg">
              <div className="mb-6">
              ...
              {asset.requiresPurchaserKYC && (
                <div className="flex items-start gap-2 p-3 bg-amber-900/10 border border-amber-900/30 rounded-lg mb-6">
              ...
              <button
                ...
                className="w-full py-3 bg-white text-black font-normal rounded-lg
      ...
      {showImageModal && (
        <div className="fixed inset-0 bg-black/95 z-50 flex items-center justify-center">
          <button
            ...
            className="absolute top-6 right-6 p-3 bg-white/10 hover:bg-white/20 rounded-lg transition-colors"
          >
