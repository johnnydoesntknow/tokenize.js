...
const PropertyModal = ({ property, onClose, onPurchaseSuccess }) => {
 ...
 const propertyData = useMemo(() => {
  ...
  const usdEquivalent = totalCost * OPN_USD_RATE;
  
  // Calculate percentage from USD amount (Weighted only)
  const calculatePercentageFromUSD = (usd) => {
    if (!usd || usd <= 0) return 0;
    const usdValue = parseFloat(usd);
    const opnNeeded = usdValue / OPN_USD_RATE;
    const percentage = (opnNeeded / propertyData.totalValue) * 100;
    return Math.min(propertyData.maxWeight || 100, percentage);
  };
  ...
  const handlePercentageSelect = (value) => {
    ...
    // Auto-update USD when percentage selected
    const costInOPN = (value / 100) * propertyData.totalValue;
    const usdValue = costInOPN * OPN_USD_RATE;
  ...
  const handleCustomInputChange = (e) => {
    ...
    if (!isNaN(numValue) && numValue > 0) {
      if (isWeightedModel) {
        ...
        // Auto-update USD when percentage changes
        const costInOPN = (clampedValue / 100) * propertyData.totalValue;
        const usdValue = costInOPN * OPN_USD_RATE;
        setUsdAmount(usdValue.toFixed(2));
      } else {
        const maxShares = Math.min(
    ...
    // Handle USD input (Weighted only)
  // Handle USD input - updates percentage automatically (Weighted only)
  const handleUSDInputChange = (e) => {
    ...
    const numValue = parseFloat(value);
    if (!isNaN(numValue) && numValue > 0) {
      const calculatedPercentage = calculatePercentageFromUSD(numValue);
                    ...
                    <div>
                      ...
                      <div className="flex gap-2">
                        <input
                          ...
                          className="flex-1 bg-black border border-neutral-800 rounded-lg px-4 py-3 text-white 
                                   focus:outline-none focus:border-white transition-colors"
                          disabled={loading || fetchingShares}
                        />
                        <div className="px-4 py-3 bg-black border border-neutral-800 rounded-lg text-neutral-400 
                                      flex items-center">
                        ...
                        <button
                          onClick={() => {
                            if (isWeightedModel) {
                              ...
                              const costInOPN = (maxPct / 100) * propertyData.totalValue;
                              setUsdAmount((costInOPN * OPN_USD_RATE).toFixed(2));
                            } else {
                              const maxShares = Math.min(
                                propertyData.availableShares,
                                propertyData.maxPurchaseAmount > 0 
