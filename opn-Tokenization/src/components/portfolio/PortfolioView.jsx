...
const fetchUserSecondaryMarketSales = async (secondaryMarket, address) => {
  if (!secondaryMarket || !address) return [];

  try {
    const filter = secondaryMarket.filters.PositionSold(null, null, null);
    
    const currentBlock = await secondaryMarket.provider.getBlockNumber();
    
    const fromBlock = Math.max(0, currentBlock - 10000);
    
    const events = await secondaryMarket.queryFilter(filter, fromBlock, currentBlock);
    
    const userSales = events.filter(event => 
      event.args.seller.toLowerCase() === address.toLowerCase()
    );
    
    return userSales.map(event => ({
    ...
    console.error('Error fetching secondary market sales:', error);
    return [];
  }
};
...
const PortfolioView = () => {
  ...
  const fetchUserData = useCallback(async () => {
    ...
    try {
      ...
      const secondaryMarketSales = await fetchUserSecondaryMarketSales(secondaryMarket, address);
      
      console.log('📊 Secondary Market Sales:', secondaryMarketSales);
      ...
      const holdingsPromises = assetIds.map(async (assetId) => {
        try {
          ...
          let realizedGainsFromSecondary = 0;
          ...
          for (const posId of positionIds) {
            ...
            const salesForThisPosition = secondaryMarketSales.filter(
              sale => sale.positionId === posId.toString()
            );
            
            if (salesForThisPosition.length > 0) {
               console.log(`Position ${posId} has ${salesForThisPosition.length} sales`);
              
              const totalSoldAmount = salesForThisPosition.reduce(
                (sum, sale) => sum.add(sale.amount), 
                ethers.BigNumber.from(0)
              );
              
              const totalProceeds = salesForThisPosition.reduce(
                (sum, sale) => sum + sale.price,
                0
              );
              
              const originalAmount = amount.add(totalSoldAmount);
              
              const proportionalPurchasePrice = purchasePrice
                .mul(amount)
                .div(originalAmount);
              
              const soldSharesCostBasis = parseFloat(
                ethers.utils.formatEther(
                  purchasePrice.mul(totalSoldAmount).div(originalAmount)
                )
              );
              
              const realizedGain = totalProceeds - soldSharesCostBasis;
              realizedGainsFromSecondary += realizedGain;
              
              console.log(`Position ${posId} adjustment:`, {
                originalAmount: originalAmount.toString(),
                remainingAmount: amount.toString(),
                soldAmount: totalSoldAmount.toString(),
                originalPurchasePrice: ethers.utils.formatEther(purchasePrice),
                adjustedPurchasePrice: ethers.utils.formatEther(proportionalPurchasePrice),
                proceeds: totalProceeds,
                costBasis: soldSharesCostBasis,
                realizedGain
              });
              
              totalAmount = totalAmount.add(amount);
              totalPurchaseValue = totalPurchaseValue.add(proportionalPurchasePrice);
            } else {
              totalAmount = totalAmount.add(amount);
          ...
          const unrealizedGain = currentVal - purchaseVal;
          const totalGain = unrealizedGain + realizedGainsFromSecondary;
          const gainPercentage = purchaseVal > 0 ? (totalGain / purchaseVal) * 100 : 0;
          
          console.log(`Asset ${assetId} final calculation:`, {
            currentValue: currentVal,
            purchaseValue: purchaseVal,
            unrealizedGain,
            realizedGainsFromSecondary,
            totalGain,
  ...
  }, [assetRegistry, positionNFT, address, isConnected, secondaryMarket]);
  ...
  const totalRealizedGains = userAssets.reduce((sum, asset) => sum + (asset.realizedGainsFromSecondary || 0), 0);
  ...
  return (
    <div className="min-h-screen bg-black relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-8 py-12">
        ...
        {/* Portfolio Summary */}
        <div className="bg-black border border-neutral-900 rounded-lg p-8 mb-8">
            ...
            <div>
              ...
              {totalRealizedGains !== 0 && (
                <p className="text-xs text-neutral-500 mt-1">
                  Realized: {totalRealizedGains >= 0 ? '+' : ''}{formatNumber(totalRealizedGains)} OPN
                </p>
              )}
            </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          <div className="bg-black border border-neutral-900 rounded-lg p-6">
              ...
              {userAssets.length === 0 ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                  {userAssets.map((asset) => (
                    <div
                      key={asset.assetId}
                      className="bg-black border border-neutral-900 rounded-lg overflow-hidden hover:border-neutral-700 transition-all group"
                    >
                      <div className="aspect-video bg-neutral-900 overflow-hidden relative">
                        ...
                        <div className="absolute top-3 right-3">
                          <div className={`px-3 py-1 rounded-lg text-xs font-normal ${
                            asset.gain >= 0 
                        ...
                        <div className="flex flex-col gap-2">
                          ...
                          {positionListings[asset.assetId] && positionListings[asset.assetId].length > 0 && (
                            <button
                              ...
                              className="w-full py-2 bg-yellow-500/10 hover:bg-yellow-500/20 border border-yellow-500/30 text-yellow-500 rounded-lg text-sm font-normal transition-colors flex items-center justify-center gap-2"
                            >
                              {cancellingListing ? (
                                <>
