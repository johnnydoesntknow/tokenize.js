...
const ComplianceView = () => {
  ...
  useEffect(() => {
    const checkRole = async () => {
      ...
      try {
        ...
        const hasRole = await Tokenization.hasRole(COMPLIANCE_ROLE, address);
  ...
  const fetchPendingRequests = async () => {
    try {
      const pendingIds = await Tokenization.getPendingRequests();
  ...
  const handleApprove = async (requestId) => {
    try {
      ...
      const tx = await Tokenization.approveRequest(requestId);
      await tx.wait();
  ...
  const handleReject = async (requestId) => {
    ...
    try {
      setProcessingId(requestId);
      const tx = await Tokenization.rejectRequest(requestId, reason);
      await tx.wait();
                ...
                <div className="flex items-start space-x-8">
                  ...
                  <div className="flex-1">
                    <div className="flex items-start justify-between mb-6">
                      ...
                      <span className="px-3 py-1 text-xs uppercase tracking-wider rounded-lg 
